...
dag = DAG('itunes_store_checker_amer',
  description = 'pulling itunes storechecker data into BQ for US Region',
  schedule_interval='30 16 * * *',
  max_active_runs=3,
  default_args=default_args)

#gcs_sensor_itunes_amer = GoogleCloudStorageObjectSensor(
#    task_id='gcs_sensor_itunes_amer',
#    bucket='gs://umg-comm-tech-dev/apple_music_tool_reports/AMER',
#    object='ITUNESDESKTOP-US-{{macros.ds_format(macros.ds_add( ds, 1),\'%Y-%m-%d\',\'%Y%m%d\')}}.csv',
#    google_cloud_conn_id='google_cloud_storage_default',
#    dag=dag)

make_dir_worker = BashOperator(
        task_id='make_dir_worker',
        bash_command='mkdir -m 777 -p /home/airflow/gcs/data/itunes/AMER/date={{macros.ds_format(macros.ds_add( ds, 1),\'%Y-%m-%d\',\'%Y%m%d\')}}',
        dag=dag
        )

download_from_gcs = BashOperator(
        task_id='download_from_gcs',
        bash_command='gsutil cp gs://umg-comm-tech-dev/apple_music_tool_reports/AMER/*ITUNESDESKTOP*{{macros.ds_format(macros.ds_add( ds, 1),\'%Y-%m-%d\',\'%Y%m%d\')}}*.csv /home/airflow/gcs/data/itunes/AMER/date={{macros.ds_format(macros.ds_add( ds, 1),\'%Y-%m-%d\',\'%Y%m%d\')}}/',
        dag=dag
        )
...
join_csv_itunes = BashOperator(
        ...
        bash_command='python /home/airflow/gcs/dags/app/itunes_join_files_v3.py /home/airflow/gcs/data/itunes/AMER/date={{macros.ds_format(macros.ds_add( ds, 1),\'%Y-%m-%d\',\'%Y%m%d\')}}/ *ITUNESDESKTOP*{{macros.ds_format(macros.ds_add( ds, 1),\'%Y-%m-%d\',\'%Y%m%d\')}}*.csv /home/airflow/gcs/data/itunes/AMER/date={{macros.ds_format(macros.ds_add( ds, 1),\'%Y-%m-%d\',\'%Y%m%d\')}}/ITUNES{{macros.ds_format(macros.ds_add( ds, 1),\'%Y-%m-%d\',\'%Y%m%d\')}}.csv',
        dag=dag
        )

upload_joined_files = BashOperator(
        task_id='upload_joined_files',
        bash_command='gsutil cp /home/airflow/gcs/data/itunes/AMER/date={{macros.ds_format(macros.ds_add( ds, 1),\'%Y-%m-%d\',\'%Y%m%d\')}}/ITUNES{{macros.ds_format(macros.ds_add( ds, 1),\'%Y-%m-%d\',\'%Y%m%d\')}}.csv gs://umg-comm-tech-dev/store_checker/AMER/date_id={{macros.ds_format(macros.ds_add( ds, 1),\'%Y-%m-%d\',\'%Y%m%d\')}}/',
        dag=dag
        )
...
load_to_itunes_bq = gcs_to_bq.GoogleCloudStorageToBigQueryOperator(
    ...
    source_objects=['store_checker/AMER/date_id={{macros.ds_format(macros.ds_add( ds, 1),\'%Y-%m-%d\',\'%Y%m%d\')}}/ITUNES{{macros.ds_format(macros.ds_add( ds, 1),\'%Y-%m-%d\',\'%Y%m%d\')}}.csv'],
...
remove_files_worker = BashOperator(
        ...
        bash_command="rm -f /home/airflow/gcs/data/itunes/AMER/date={{macros.ds_format(macros.ds_add( ds, 1),\'%Y-%m-%d\',\'%Y%m%d\')}}/*.csv",
        dag = dag
)


#gcs_sensor_itunes_amer >>
