SELECT
  ...
  (skips/streams)/nullif(avgskips-1,0) AS skipsperf,
  ...
  (streams/baseline)/nullif((avgskipsb4-1),0) AS skipsb4perf
FROM
  `umg-comm-tech-dev.Optimize.Ar_TPR_Streams_Final1_Days`  f
JOIN (
  SELECT
    source_uri,
    AVG(skips/streams) AS avgskips,
    AVG(length) AS avglength,
    AVG(disc) AS avgdisc,
    AVG(col) AS avgcol,
    AVG(ret) AS avgret,
    AVG(sup) AS avgsup,
    AVG(streams/baseline) AS avgskipsb4
  FROM
