...
import json
import os
import pickle # Only useful in Python 2.7
import operator
...
class Data_prep():
    ...
    def prepare_data(self):
        self.isrc_lookup = self.playlists_data[['isrc', 'track_artist', 'track_title']].drop_duplicates()  # Only get unique isrcs
        self.isrc_lookup['isrc'] = self.isrc_lookup.isrc.astype(str)  # Encode as string
        self.playlists = list(np.sort(self.playlists_data.playlist_uri.unique()))  # Get our unique playlists
        self.isrcs = list(self.playlists_data.isrc.unique())  # Get our unique isrcs
        self.dop = list(self.playlists_data.days_on_playlist)  # days on playlist
        self.rows = self.playlists_data.playlist_uri.astype(pd.api.types.CategoricalDtype( categories = self.playlists)).cat.codes  # Get the associated row indices
        self.cols = self.playlists_data.isrc.astype(pd.api.types.CategoricalDtype( categories = self.isrcs)).cat.codes  # Get the associated column indices
        self.playlists_sparse = sparse.csr_matrix((self.dop, (self.rows, self.cols)), shape=(len(self.playlists), len(self.isrcs)))
    ...
    def make_trainset(self,ratings, pct_test=0.05, rseed=0):
        '''
...
class Recommender_core():
    ...
    def train(self, playlists_trainset, factors, regularization,iterations,alpha):
        self.model = implicit.als.AlternatingLeastSquares(factors=factors, regularization=regularization, iterations=iterations)
        self.model.fit((playlists_trainset.transpose() * alpha).astype('double'))
        self.playlist_vecs =  self.model.user_factors
        self.isrc_vecs = self.model.item_factors

    def recommend(self,playlist_uri, playlist_isrc_matrix, N=10):
        recommend_start_time = time.time()
        '''Returns a list of tuple (item_index, score, target playlist) of recommended tracks'''
        plist_index = self.playlists.index(playlist_uri) # get the index of the target playlist
        recommendations = self.model.recommend(plist_index, playlist_isrc_matrix,N=N) # returns list of tuples (item_index, score)
        playlist = [playlist_uri] * len(recommendations)
    ...
    def rec_to_isrc(self, recommendations):
        ...
        return [ (self.isrcs[x[0]],x[1],x[2]) for x in recommendations ] # turns the results into a list of tuples (item_index_isrc, score)
    ...
    def rec_readable(self,recommendations):
        ...
        isrcs = [x[0] for x in recommendations]
        scores = [x[1] for x in recommendations]
        artist = [self.isrc_lookup.track_artist.loc[self.isrc_lookup.isrc == x[0]].iloc[0] for x in recommendations] # get artist name for each isrc
        title  = [self.isrc_lookup.track_title.loc[self.isrc_lookup.isrc == x[0]].iloc[0] for x in recommendations]# get title for each isrc
        seed_playlist_uri = [x[2] for x in recommendations]
        ...
        return pd.DataFrame({'scores':scores,'isrc': isrcs, 'artist': artist, 'title': title,'seed_playlist_uri':seed_playlist_uri})
...
class Recommender(Data_prep, Recommender_core):
    ...
    def build_recommender(self,playlist_data_path):#,isrc_data_path,non_spotify_playlists_sample_path,spotify_playlists_sample_path):
        ...
        self.train(self.playlists_trainset, self.factors, self.regularization, self.iterations, self.alpha)
