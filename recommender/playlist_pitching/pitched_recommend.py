...
class Data_prep():
    ...
    def prepare_data(self, playlist_data_path):
        playlists_data = pd.read_csv(playlist_data_path, error_bad_lines=False)
        self.isrc_lookup = playlists_data[['isrc', 'track_artist', 'track_title']].drop_duplicates()  # Only get unique isrcs
        ...
        self.rows = playlists_data.playlist_uri.astype(pd.api.types.CategoricalDtype(categories = self.playlists)).cat.codes  # Get the associated row indices
        self.cols = playlists_data.isrc.astype(pd.api.types.CategoricalDtype(categories = self.isrcs)).cat.codes  # Get the associated column indices
...
class Recommender_core():
    ...
    def rec_to_isrc(self, recommendations):
        ...
        return [ (self.isrcs[x[0]],float(x[1]),x[2]) for x in recommendations ] # turns the results into a list of tuples (item_index_isrc, score)
